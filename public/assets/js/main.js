...
(function ($) {
	...
	$(document).ready(() => {
		...
		const headerT = $('.header-section-2');
		$(window).on('scroll', function () {
			if ($(this).scrollTop() < 1) {
		...
		$('.hover-tab ul.tab-menu li').on('mouseover', function (g) {
			const tabT = $(this).closest('.hover-tab');
			const indexT = $(this).closest('li').index();
		...
		$('.deposit-method-item').on('click', function () {
			$(this).parent('.owl-item').siblings().children('.deposit-method-item')
				.removeClass('active');
			$(this).addClass('active');
		});
		$(() => {
			$('#btc-range').slider({
				...
				max: 1000,
				slide(event, ui) {
					$('#btc-amount').val(ui.value + ' SGD');
				},
			});
			$('#btc-amount').val('SGD' + $('#btc-range').slider('value'));
		});
		$(() => {
			$('#usd-range').slider({
				range: 'min',
